...
### 9. 결정을 네 줄로 압축하고 마지막 한 걸음을 실행하기

모델이 정확한 다음 단계를 이미 알면서도 새 조사, 새 child, 새 도구로
옆으로 확장하다 끝나는 경우가 반복됐습니다.

이제 중요한 결과가 나올 때마다 네 줄만 갱신합니다. 현재 경계(frontier),
마지막 의미 있는 결과, 확인하지 않은 가장 싼 실험, 그리고 바로 다음에
실행할 정확한 동작입니다. 이 네 줄의 "바로 다음 동작"이 실제 실행되어
결과를 내거나 불가능함이 확인되기 전에는 새 조사·child·도구를 열어도
진전으로 인정하지 않습니다.

특히 하나의 능력(capability)이 실제로 증명되고, 남은 것이 성공하면 답
후보나 확실한 반증으로 이어지는 딱 한 번의 실행뿐이라면 탐색을 멈춥니다.
같은 다리(bridge)를 다른 언어나 도구로 다시 만드는 것도 새 진전이
아니며, 남은 그 한 번을 실행해야 합니다.

### 10. 제어 거부는 딱 한 번 고치고, 종료는 상태가 바뀐 뒤에 말하기

모델은 거부된 요청을 이름만 바꿔 같은 것을 계속 다시 시도했고,
"이제 멈추겠다"고 말해 놓고 계속 일했습니다.

제어 평면(control plane)의 거부는 돌려받은 복구 방법으로 정확히 한 번만
고칩니다. 그래도 막히면 그 길을 떠나 대상 자체에 대한 다른 실험이나
마무리 절차로 갑니다. 가족(family)이나 표현(representation) 이름을
바꾸는 것, 같은 명령을 patch·인라인 코드·child·curl로 다시 포장하는
것, 상태가 안 바뀐 채 status 조회를 반복하는 것은 진전이 아닙니다.

또한 후보 저장, 정답 확인, child 정리, 자원 정리 같은 종료 동작이 실제로
실행되기 전에는 "완료했다", "이제 멈춘다" 같은 종료 선언을 하지 않습니다.

### 11. child는 빌려준 것이므로 반드시 회수하기

하나의 의미 경계(frontier)에서 미회수 child는 최대 하나만 허용합니다.
같은 경계에서 child를 더 열기 전에 root는 그 child의 결과를 정확한
증거 ID로 직접 읽고, 필요하면 결정적인 관찰을 스스로 다시 실행하고,
보존할지 버릴지 판단한 뒤 그 child를 닫거나 명시적으로 인계합니다.

서로 다른 경계라면 child를 병렬로 써도 됩니다. 문제는 같은 주제에서
결과를 정산하지 않은 채 child만 늘리는 것입니다.

### 12. 외부 검토로 넘겨서는 안 되는 경우 구분하기

섹션 7의 조건에 더해, 다음은 외부 검토를 부르는 "막힘"이 아닙니다.
명령 중재기(mediator)가 거부한 것, schema·사전 검사 거부, 폴더나 파일이
없어 분석이 시작조차 못한 실패, 대상 실행 전의 시간 초과, 출력이 안
보여서 결과를 판단할 수 없는 경우입니다. 이런 것들은 대상이 막힌 게
아니라 실행 환경이 막힌 것입니다.

외부 검토를 요청하려면 두 차례 모두 대상이 실제로 실행됐고, 두 결과의
의미 지문(fingerprint)이 같으며, 그 사이에 관점을 바꾼 재현이 없었다는
것을 각 차례의 정확한 증거 ID와 함께 먼저 보여줘야 합니다.

### 13. 원격 상호작용 전에 실행 경계 계산하기

여러 번 요청과 응답을 주고받는 원격 exploit은 풀이가 맞아도 서비스 수명보다
오래 걸리면 실패합니다. 실행 전에 실제 관찰로 다음 다섯 가지를 한 장에
기록합니다.

- 프로세스 또는 서비스가 유지되는 시간
- 예상 왕복 횟수와 걸리는 시간
- timeout 위험
- 이전 응답과 무관해 미리 pipeline할 수 있는 단계
- 짧은 read, binary payload, parser 상태처럼 응답을 기다려야 하는 경계

서비스 수명을 모르면 전체 exploit을 실행하기 전에 새 연결 하나로 먼저
측정합니다. 이전 응답과 무관한 입력만 묶어 보내고, 다음 입력이 응답 bytes나
상태에 따라 달라지는 경계에서는 반드시 기다립니다.

### 14. 두 번째 무정보 결과에는 차단 대신 정확한 경고하기

같은 의미의 실험이 두 번 새 정보를 만들지 못하면 반복 횟수, 새로 생긴
capability·candidate·bound가 없다는 사실, 아직 쓰지 않은 관찰 자료, 가장
싼 다른 방법을 한 번 보여줍니다.

이 경고는 read-only 조사나 실제로 달라진 실험을 막지 않습니다. 하드 차단은
중복 외부 전송, 권한, terminal, lifecycle처럼 정당한 예외가 없는 좁은
불변식에만 사용합니다.

### 15. Pwn 로컬 성공은 실제 wrapper로 다시 확인하기

로컬에서 코드 실행이 된 것처럼 보여도 debugger, `/proc`, 숨은 로컬 주소,
간소화한 실행 환경에 의존했다면 실제 문제 풀이 증거가 아닙니다. Pwn 로컬
RCE로 인정하려면 정확한 challenge wrapper, debugger 없는 새 프로세스,
`/proc` 없는 환경에서 실제 원격 실행에도 쓸 같은 solver가 flag 형태의
결과를 내야 합니다.

그 다음에만 측정한 서비스 수명과 예상 왕복 비용을 비교하고, 같은 solver를
변경하지 않은 채 실제 acceptance surface에서 확인합니다.
